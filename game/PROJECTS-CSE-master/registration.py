...
import subprocess
def run():
    subprocess.call(["python","average.py"])   
def sun():
    subprocess.call(["python","easy.py"]) 
def dun():
    subprocess.call(["python","Hard.py"]) 
def feed():
    subprocess.call(["python","FEDBACK.py"])    
root=Tk()
class first():
        ...
        Button(root, text='Submit and Start Quiz in easy mode',width=30,bg='brown',fg='white',command=sun).place(x=180,y=380)
        
        Button(root, text='Submit and Start Quiz in average mode',width=30,bg='brown',fg='white',command=run).place(x=180,y=410)

        Button(root, text='Submit and Start Quiz in hard mode',width=30,bg='brown',fg='white',command=dun).place(x=180,y=440)

root.mainloop()
Button(root, text='Give Feedback',width=30,bg='brown',fg='white',command=feed).place(x=180,y=380)
